<?php
/**
 * Taxonomy template for forum flairs.
 *
 * @package gta6modswp
 */
...
$top_range_select_id  = 'forum-top-range-flair';
$show_top_time_range  = ('top' === $initial_sort);

$heading      = sprintf(__('Threads tagged with "%s"', 'gta6mods'), $term->name);
$description_markup = gta6_forum_get_flair_description_markup($term);
...
if (!empty($flairs) && !is_wp_error($flairs)) {
    foreach ($flairs as $flair_item) {
        $flair_name = wp_strip_all_tags($flair_item->name);
        $flair_metadata[$flair_item->slug] = [
            'heading'     => sprintf(__('Threads tagged with "%s"', 'gta6mods'), $flair_name),
            'description' => gta6_forum_get_flair_description_markup($flair_item),
        ];
    }
}
...
<main class="container mx-auto px-4 lg:px-6 mt-8" data-forum-main data-base-url="<?php echo esc_url($forum_home_url); ?>" data-flair-path="<?php echo esc_attr($flair_path); ?>" data-flair-base-url="<?php echo esc_url($flair_base_url); ?>" data-initial-flair="<?php echo esc_attr($term->slug); ?>" data-initial-search="<?php echo esc_attr($search_term); ?>" data-initial-sort="<?php echo esc_attr($initial_sort); ?>" data-initial-time-range="<?php echo esc_attr($initial_time_range); ?>" data-initial-page="<?php echo esc_attr((string) $initial_current_page); ?>" data-initial-total-pages="<?php echo esc_attr((string) $initial_total_pages); ?>" data-per-page="<?php echo esc_attr((string) $threads_per_page); ?>" data-has-initial="1" data-flair-metadata="<?php echo esc_attr($flair_metadata_json); ?>">
    <div class="flex items-start gap-3 mb-6">
        <a href="<?php echo esc_url($forum_home_url); ?>" class="forum-back-button text-lg md:text-4xl inline-flex -mt-3 items-center p-2 md:p-3 rounded-full hover:bg-gray-200 justify-center text-black transition" aria-label="<?php echo esc_attr__('Back to forum home', 'gta6mods'); ?>">
            <i class="fas fa-chevron-left" aria-hidden="true"></i>
        </a>
        <div class="flex-1">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900 mb-2" data-forum-heading><?php echo esc_html($heading); ?></h1>
            <div class="text-sm text-gray-500 mb-4 flair-description" data-forum-description><?php echo $description_markup; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
        </div>
    </div>
    <div class="grid grid-cols-12 gap-6">
        <div class="col-span-12 lg:col-span-8">
            <div class="card p-3 mb-4 flex items-center gap-3">
                <img src="<?php echo esc_url(get_avatar_url(get_current_user_id(), ['size' => 80])); ?>" class="rounded-full w-10 h-10" alt="<?php echo esc_attr__('Your avatar', 'gta6mods'); ?>">
                <a href="<?php echo esc_url($create_thread_url); ?>" class="flex-grow bg-gray-100 hover:bg-gray-200 border border-gray-300 hover:border-gray-400 text-left px-4 py-2 rounded-md text-gray-600 transition">
                    <?php echo esc_html__('Start a new thread…', 'gta6mods'); ?>
                </a>
                <a href="<?php echo esc_url($image_thread_url); ?>" class="p-2 text-gray-500 hover:text-pink-600 hover:bg-pink-100 rounded-md transition" aria-label="<?php echo esc_attr__('Create a media thread', 'gta6mods'); ?>">
                ...
                <a href="<?php echo esc_url($link_thread_url); ?>" class="p-2 text-gray-500 hover:text-pink-600 hover:bg-pink-100 rounded-md transition" aria-label="<?php echo esc_attr__('Create a link thread', 'gta6mods'); ?>">
            ...
            <div class="space-y-4" data-thread-list>
                <?php foreach ($initial_threads as $thread_item) : ?>
                    <?php gta6_forum_render_thread_card($thread_item); ?>
                <?php endforeach; ?>
            </div>
                    ...
                    <div class="p-4">
                        <div class="flex flex-wrap gap-2">
                            <?php if (!empty($flairs) && !is_wp_error($flairs)) : ?>
                                <?php foreach ($flairs as $flair_item) : ?>
                                    <?php
                                    $colors     = gta6_forum_get_flair_colors($flair_item->term_id);
                                    $flair_link = get_term_link($flair_item, 'forum_flair');
                                    if (is_wp_error($flair_link)) {
                                        $flair_link = '';
                                    }
                                    $is_active = $flair_item->term_id === $term->term_id;
                                    ?>
                                    <a href="<?php echo esc_url($flair_link); ?>" class="flair-filter-tag post-flair<?php echo $is_active ? ' selected' : ''; ?>" data-flair-filter data-flair="<?php echo esc_attr($flair_item->slug); ?>" style="background-color: <?php echo esc_attr($colors['background']); ?>; color: <?php echo esc_attr($colors['text']); ?>;" rel="tag">
                                        <?php echo esc_html($flair_item->name); ?>
                                    </a>
                                <?php endforeach; ?>
                            <?php else : ?>
