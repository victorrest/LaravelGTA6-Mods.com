            ...
            <aside class="space-y-6">
                <div class="card p-6 space-y-4">
                    ...
                    @if (auth()->check() && auth()->id() === $mod->user_id)
                        ...
                        @endphp
                        <button onclick="togglePinMod({{ $mod->id }}, @json($isPinned))" id="pin-mod-btn" class="w-full inline-flex items-center justify-center px-4 py-2 {{ $isPinned ? 'bg-purple-600 hover:bg-purple-700' : 'bg-gray-600 hover:bg-gray-700' }} text-white text-sm font-semibold rounded-lg shadow transition">
